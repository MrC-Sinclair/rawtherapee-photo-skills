...
>
> 本文件随 `rawtherapee-photo-skills` 合并包发布（摘自 `openclaw-photo-agents-creator/templates/`），
> 供 `photo-grader/scripts/grade.py` 报错时指引 JSON schema。末段
> `{{RT_MAPPING_REFERENCE}}` 占位符由同目录的 `rt-mapping-reference.md` 内容填充。
